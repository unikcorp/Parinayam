...
export function ProfileCard({
  ...
  return (
    <div
      className={cn(
        "group relative rounded-3xl transition-all duration-200 ease-out hover:-translate-y-1.5 hover:shadow-card-hover",
        highlighted
    ...
    >
      <div
        className={cn(
          ...
          highlighted ? "rounded-[21px]" : "rounded-3xl"
        )}
      >
        <CardLink memberId={memberId} teaser={teaser}>
          <div
            className={cn("relative h-[210px] w-full shrink-0 overflow-hidden", photoClassName)}
          >
            <MemberProfilePhoto
            ...
            />
            <HighlightRibbon isHighlighted={!!highlighted} radiusClassName="rounded-t-[21px]" />
            <PremiumBadge
              isPremium={!!premium}
              className={cn(
                "animate-pop absolute left-3 z-10",
                highlighted ? "top-8" : "top-3"
              )}
            />
            {online && (
