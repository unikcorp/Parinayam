  ...
  return (
    <div className="flex overflow-hidden rounded-[18px] border border-card-border bg-card shadow-[0_4px_16px_rgba(127,29,29,0.05)]">
      <div className="relative w-31 shrink-0">
        ...
        {result.online && (
          <span className="absolute top-2 left-2 size-2.5 rounded-full border-2 border-white bg-success" />
        )}
      </div>
      <div className="flex flex-1 flex-col p-3.5">
        <div className="flex items-center gap-1.5">
          ...
          {result.premium && <Star className="size-3 fill-current text-gold" />}
        </div>
        <div className="mt-0.5 truncate text-[12.5px] text-muted-foreground">{result.job}</div>
